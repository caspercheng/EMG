...
<!---更新維護檔的尾款實收金額--->
	<cfloop query="COPKA">
		...
		<!---<cfloop query="COPTD">
			...
			<cfif #COPTD.RECORDCOUNT# GT 0>		
				<cfloop query="COPTD">		
					<cfset 尾款預收日=#尾款預收日#&#MID(TA020,1,4)#&"-"&#MID(TA020,5,2)#&"-"&#MID(TA020,7,2)#&"<BR>">	
				</cfloop>									
			<cfelse>
...
<cfif #FORM.SUBMIT# EQ "尾款回推">
	...
	SELECT 	ROUND(ACR.TD014*ACRTB.UDF06,2) 收款金額,
	...
	<cfloop query="ACRTC">
		...
		<cfquery name="COPKC_INSERT" datasource="#SESSION.COMPANY#">	
			...
			VALUES 
			('#SN#','#TD001#','#TD002#','#TD003#',#收款金額#,'#收款日期#')
		</cfquery>	
	</cfloop>
