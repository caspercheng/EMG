...
<cfif #FORM.SUBMIT# EQ "更新">
	...
	<cfset ETA="">	
		...
		WHERE TB004+TB005+TB006 IN 
		(SELECT TD001+TD002+TD003
		FROM COPTD
		WHERE 1=1
			AND TD001='#KA001#'
...
<!---更新維護檔的尾款實收金額--->
	<cfloop query="COPKA">
		...
		<cfquery name="COPTH_DATE" datasource="#SESSION.COMPANY#">
			...
			AND TH020='Y'
			GROUP BY convert(datetime, TG003, 21)+CMSNA.UDF06  
			order by convert(datetime, TG003, 21)+CMSNA.UDF06  
		</cfquery>
		
		<cfif #NA002# EQ "37">
			<cfif  #mid(尾款預收日_change,1,10)# NEQ "1900-01-01" AND #尾款預收日_change# NEQ "">		
				<cfif #dayofweek(尾款預收日_change)# eq 1><cfset 尾款預收日=尾款預收日_change+2></cfif>
				<cfif #dayofweek(尾款預收日_change)# eq 2><cfset 尾款預收日=尾款預收日_change+8></cfif>
				<cfif #dayofweek(尾款預收日_change)# eq 3><cfset 尾款預收日=尾款預收日_change+7></cfif>
				<cfif #dayofweek(尾款預收日_change)# eq 4><cfset 尾款預收日=尾款預收日_change+6></cfif>
				<cfif #dayofweek(尾款預收日_change)# eq 5><cfset 尾款預收日=尾款預收日_change+5></cfif>
				<cfif #dayofweek(尾款預收日_change)# eq 6><cfset 尾款預收日=尾款預收日_change+4></cfif>
				<cfif #dayofweek(尾款預收日_change)# eq 7><cfset 尾款預收日=尾款預收日_change+3></cfif>
			<cfelseif #KA021# NEQ "1900-01-01" AND #KA021# NEQ "">
				<cfif #dayofweek(尾款預收日_first)# eq 1><cfset 尾款預收日=尾款預收日_first+2></cfif>
				<cfif #dayofweek(尾款預收日_first)# eq 2><cfset 尾款預收日=尾款預收日_first+8></cfif>
				<cfif #dayofweek(尾款預收日_first)# eq 3><cfset 尾款預收日=尾款預收日_first+7></cfif>
				<cfif #dayofweek(尾款預收日_first)# eq 4><cfset 尾款預收日=尾款預收日_first+6></cfif>
				<cfif #dayofweek(尾款預收日_first)# eq 5><cfset 尾款預收日=尾款預收日_first+5></cfif>
				<cfif #dayofweek(尾款預收日_first)# eq 6><cfset 尾款預收日=尾款預收日_first+4></cfif>
				<cfif #dayofweek(尾款預收日_first)# eq 7><cfset 尾款預收日=尾款預收日_first+3></cfif>
			<cfelseif #COPTH_DATE.recordcount# gt 0>
					<cfloop query="COPTH_DATE">
						<cfif #dayofweek(計算後日期)# eq 1><cfset 計算後日期=計算後日期+2></cfif>
						<cfif #dayofweek(計算後日期)# eq 2><cfset 計算後日期=計算後日期+8></cfif>
						<cfif #dayofweek(計算後日期)# eq 3><cfset 計算後日期=計算後日期+7></cfif>
						<cfif #dayofweek(計算後日期)# eq 4><cfset 計算後日期=計算後日期+6></cfif>
						<cfif #dayofweek(計算後日期)# eq 5><cfset 計算後日期=計算後日期+5></cfif>
						<cfif #dayofweek(計算後日期)# eq 6><cfset 計算後日期=計算後日期+4></cfif>
						<cfif #dayofweek(計算後日期)# eq 7><cfset 計算後日期=計算後日期+3></cfif>
						<cfset 尾款預收日=#尾款預收日#&#MID(計算後日期,1,10)#&"<br>">	
					</cfloop>
			<cfelse>
				...
				<cfloop query ="CMSNA">
						<cfif #dayofweek(尾款預收日)# eq 1><cfset 尾款預收日=尾款預收日+2></cfif>
						<cfif #dayofweek(尾款預收日)# eq 2><cfset 尾款預收日=尾款預收日+8></cfif>
						<cfif #dayofweek(尾款預收日)# eq 3><cfset 尾款預收日=尾款預收日+7></cfif>
						<cfif #dayofweek(尾款預收日)# eq 4><cfset 尾款預收日=尾款預收日+6></cfif>
						<cfif #dayofweek(尾款預收日)# eq 5><cfset 尾款預收日=尾款預收日+5></cfif>
						<cfif #dayofweek(尾款預收日)# eq 6><cfset 尾款預收日=尾款預收日+4></cfif>
						<cfif #dayofweek(尾款預收日)# eq 7><cfset 尾款預收日=尾款預收日+3></cfif>
					<cfset 尾款預收日 =#MID(尾款預收日,1,10)#></cfloop>
			</cfif>
		...
		<cfelse>
			...
			<cfif  #mid(尾款預收日_change,1,10)# NEQ "1900-01-01" AND #尾款預收日_change# NEQ "">		
					<cfset 尾款預收日=#MID(尾款預收日_change,1,10)#>	
			<cfelseif  #KA021# NEQ "1900-01-01" AND #KA021# NEQ "">		
					<cfset 尾款預收日=#MID(尾款預收日_first,1,10)#>	
			<cfelseif #COPTH_DATE.recordcount# gt 0>
					<cfloop query="COPTH_DATE">
						<cfset 尾款預收日=#尾款預收日#&#MID(計算後日期,1,10)#&"<br>">	
					</cfloop>	
			<cfelse>
				...
				<cfloop query ="CMSNA"><cfset 尾款預收日 =#MID(尾款預收日,1,10)#></cfloop>
...
--->	
	...
	WHERE 1=1 
		...
		<cfquery name="KA020_UPDATE" datasource="#SESSION.COMPANY#">
			UPDATE COPKA
					SET 	KA020=ROUND(((KA016+KA013) / KA008),4)
			FROM COPKA
