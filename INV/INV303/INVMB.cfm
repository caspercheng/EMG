...
<cfif URL.MB001 IS NOT ""> <cfset #FORM.MB001# = #URL.MB001#></cfif>
<cfif URL.submit IS NOT ""> <cfset #FORM.submit# = #URL.submit#></cfif>

<cfif NOT IsDefined("FORM.d1")><cfset #FORM.d1#="#DATEFORMAT(now()-366,"yyyymm")#"></cfif> 
<cfif NOT IsDefined("FORM.d2")><cfset #FORM.d2#="#DATEFORMAT(now()-30,"yyyymm")#"></cfif> 
...
<cfif NOT IsDefined("FORM.MB025")><cfset #FORM.MB025#=""></cfif> 
...
<cfform action="INVMB.cfm" method="post">
	...
	<table align="center" bgcolor="9999CC">
	 <tr>
	  <td>耗用月份</td>
	  <td><input type="text" name="d1" size="6" value="#FORM.d1#" >～</td>
	  <td><input type="text" name="d2"  size="6" value="#FORM.d2#" ></td>
	  <td>屬性</td>
	  <td>
	     <select name="MB025">
		   <cfif FORM.MB025 IS NOT ""> <option value="#FORM.MB025#">#FORM.MB025#</option></cfif>
		   <option value="">全部</option>
		   <option value="P">採購件</option>
		   <option value="S">加工件</option>
		  </select>
	  </td>               
	  <td>品號</td><td><cfinput type="text" size="15" name="MB001" value="#FORM.MB001#"></td>
	  <td>品名</td><td><cfinput type="text" size="15" name="MB002" value="#FORM.MB002#"></td>
	  <td>規格</td><td><cfinput type="text" size="15" name="MB003" value="#FORM.MB003#"></td>

	  <td>庫存數量</td>
	  <td>
	     <select name="MB064">
		   <option value="">全部</option>
		   <option value="K">僅秀有庫存品號</option>
		  </select>
	  </td>               
	  
	  <td>筆數</td><td><select name="num"><option value="#FORM.num#">#FORM.num#</option><option value="500">500</option><option value="1000">1000</option><option value="2000">2000</option></select></td>               
...
<cfset begin_mon= #mid(FORM.d1,1,4)#&'-'&#mid(FORM.d1,5,2)#&'-01'>
<cfset end_mon2= #mid(FORM.d2,1,4)#&'-'&#mid(FORM.d2,5,2)#&'-01'>
<cfset lastday=#DaysInMonth(end_mon2)#>
<cfset  end_mon= #mid(FORM.d2,1,4)#&'-'&#mid(FORM.d2,5,2)#&'-'&#lastday#>
<cfset mon =#DateDiff("M", begin_mon, end_mon)#>

<cfquery datasource="#SESSION.COMPANY#" name="INVMB">
    SELECT  top #FORM.num# MB001,MB002,MB003,MB036,MB037,MB039,MB064,MB065,MC004,
    ...
    LEFT JOIN
	 (SELECT TH004,SUM(TH008-TH043) AS COP_TH008 
	  ...
	  WHERE TH020 ='Y' 
	   AND TG003 >= '#FORM.d1#01'
	   AND TG003 <= '#FORM.d2##lastday#'
    ...
    LEFT JOIN
	 (SELECT TE004,SUM(TE005) AS MOC_TE005
	  ...
	  WHERE TE019 ='Y' 
	   AND TC003 >= '#FORM.d1#01'
	   AND TC003 <= '#FORM.d2##lastday#'
    ...
    <cfif FORM.MB002 IS NOT ""> AND  MB002 like N'%#FORM.MB002#%' </cfif>
    <cfif FORM.MB003 IS NOT ""> AND  MB003 like N'%#FORM.MB003#%' </cfif>
    <cfif FORM.MB025 IS NOT ""> AND  MB025 = '#FORM.MB025#' </cfif>
    <cfif FORM.MB064 IS  "K">AND MB064 > 0</cfif>
	   
	ORDER BY MB001
</cfquery>

<a href="INVMB_excel.cfm?d1=#FORM.d1#&d2=#FORM.d2#&MB001=#FORM.MB001#&MB002=#FORM.MB002#&MB003=#FORM.MB003#&MB025=#FORM.MB025#" class="btn btn-info btm-sm m-1" target="_blank">產生EXCEL</a>
...
<cfloop query="INVMB">
	...
	<cfif amount LT 0><cfset bgcolor2="FFCCFF"><cfelse><cfset bgcolor2= bgcolor></cfif>
	
	<tr bgcolor="#bgcolor#" >
		<TD align="center"><a href="/EMG/INV/ref/INVMB_detail.cfm?MB001=#trim(MB001)#" target="_blank">#MB001#</a></TD>
